      ...
      <div class="flex-none">
        <ul class="menu menu-horizontal font-bold text-shadow px-1">
          {#each links as link}
            {#if link?.children}
              <li tabindex="-1">
                <a data-sveltekit-preload-data href={link.href}>
                  {link.text}
                  <i> 
                    <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z"/></svg>
                  </i>
                </a>
                <ul class="p-3 bg-base-200 font-bold">
                  {#each link.children as child}
                    <li><a data-sveltekit-preload-data href={child.href}>{child.text}</a></li>
                  {/each}
                </ul>
              </li>
            {:else}
              {#if $page.url.pathname === link.href}
                <li><a class="opacity-70" href={link.href}>{link.text}</a></li>
              {:else}
